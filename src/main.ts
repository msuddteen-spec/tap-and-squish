...
const renderer = new THREE.WebGLRenderer({ antialias: true, powerPreference: 'high-performance' }); const mobileMode = window.matchMedia('(max-width: 600px)').matches; renderer.setPixelRatio(Math.min(window.devicePixelRatio, mobileMode ? 1.5 : 2)); renderer.outputColorSpace = THREE.SRGBColorSpace; renderer.shadowMap.enabled = !mobileMode; renderer.shadowMap.type = THREE.PCFSoftShadowMap; renderer.domElement.className = 'game-canvas'; renderer.domElement.setAttribute('aria-label', 'แตะก้อนขนมปังเพื่อบีบ'); app.appendChild(renderer.domElement);
scene.add(new THREE.HemisphereLight('#fffaf2', '#d9a875', 2.2));
const keyLight = new THREE.DirectionalLight('#fff4df', 3.2); keyLight.position.set(-3, 5, 5); keyLight.castShadow = !mobileMode; scene.add(keyLight);
...
const bread = createBread(); bread.scale.setScalar(0.72); scene.add(bread); const raycaster = new THREE.Raycaster(); const pointer = new THREE.Vector2(); const targetSquish = new THREE.Vector3(0.72, 0.72, 0.72); const targetIndent = new THREE.Vector3(); const indent = new THREE.Vector3(); let isPressing = false; let hasPressed = false;
const ui = new UIManager(app, game, analytics);
...
renderer.domElement.addEventListener('pointerdown', (event) => { event.preventDefault(); renderer.domElement.setPointerCapture(event.pointerId); updatePointer(event); raycaster.setFromCamera(pointer, camera); if (raycaster.intersectObject(bread, true).length === 0) return; game.audio.unlock(); isPressing = true; targetSquish.set(0.81, 0.53, 0.81); targetIndent.set(pointer.x * 0.32, pointer.y * 0.32, 0.18); const result = game.press();
analytics.track('bread_press', { score_added: result.scoreAdded, combo: result.combo, multiplier: result.multiplier });
if (!hasPressed) { hasPressed = true; analytics.track('first_press'); }
ui.updatePress(result, game.state, { x: event.clientX, y: event.clientY }); });
...
const releaseBread = (event: PointerEvent) => { event.preventDefault(); isPressing = false; targetSquish.set(0.72, 0.72, 0.72); targetIndent.set(0, 0, 0); }; renderer.domElement.addEventListener('pointerup', releaseBread); renderer.domElement.addEventListener('pointercancel', releaseBread); renderer.domElement.addEventListener('pointerleave', (event) => { if (isPressing) releaseBread(event); });
function createBread(): THREE.Mesh { const shape = new THREE.Shape(); shape.moveTo(-1.18, -0.85); shape.bezierCurveTo(-1.3, -0.3, -1.22, 0.4, -0.82, 0.78); shape.bezierCurveTo(-0.4, 1.2, 0.4, 1.2, 0.82, 0.78); shape.bezierCurveTo(1.22, 0.4, 1.3, -0.3, 1.18, -0.85); shape.bezierCurveTo(0.7, -1.15, -0.7, -1.15, -1.18, -0.85); const geometry = new THREE.ExtrudeGeometry(shape, { depth: 1.05, bevelEnabled: true, bevelSegments: mobileMode ? 4 : 8, steps: mobileMode ? 2 : 3, bevelSize: 0.22, bevelThickness: 0.22, curveSegments: mobileMode ? 16 : 24 }); geometry.center(); geometry.rotateX(-0.06); const mesh = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({ color: '#e7a05b', roughness: 0.82 })); mesh.position.y = 0.05; mesh.castShadow = !mobileMode; mesh.receiveShadow = !mobileMode; mesh.name = 'soft-bread'; return mesh; }
...
function animate(): void { requestAnimationFrame(animate); bread.scale.lerp(targetSquish, isPressing ? 0.22 : 0.04); indent.lerp(targetIndent, isPressing ? 0.18 : 0.05); bread.position.x = indent.x; bread.position.y = 0.05 + indent.y; bread.rotation.z = -indent.x * 0.08; bread.rotation.x = -0.06 + indent.y * 0.05; renderer.render(scene, camera); }
